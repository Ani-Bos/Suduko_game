...
function isValid( board, i,  j,  k,  n)
{
    // row check
    ...
    // submatrix check
    let rt = Math.sqrt(n);
    let start_index_i = i - i % rt;
    let start_index_j = j - j % rt;

    for (let x = start_index_i; x < start_index_i + rt; x++)
    {
        for (let y = start_index_j; y < start_index_j + rt; y++)
        {
            if (board[x][y] == k)
            {
                return false;
